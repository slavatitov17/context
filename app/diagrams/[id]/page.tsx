...
export default function DiagramDetailPage({ params }: { params: { id: string } }) {
  ...
  const diagramTypesInfo: DiagramTypeInfo[] = [
    ...
    },
    {
      type: 'ERPlantUML',
      name: 'ER (PlantUML)',
      description: 'ER диаграмма PlantUML с максимально детальными инструкциями для ИИ-модели и строгими цветами (белый, черный, серый) - гарантирует отсутствие ошибок рендеринга',
      standard: 'PlantUML',
      purpose: 'База данных',
    ...
    <div className={`flex flex-col ${isChatVisible ? '' : 'h-full'}`} style={isChatVisible ? { height: 'calc(100vh - 2rem)', overflow: 'hidden', maxHeight: 'calc(100vh - 2rem)' } : {}}>
      {!diagramType ? (
        ...
        <div>
          ...
          /* Область чата */
          <div className="flex-1 flex gap-4 min-h-0 overflow-hidden" style={{ position: 'relative', height: 'calc(100vh - 4.5rem)' }}>
            {/* Чат */}
