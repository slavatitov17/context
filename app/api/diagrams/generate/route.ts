...
export async function POST(request: NextRequest) {
  try {
    ...
    const getMermaidSyntaxInstructions = (type: string, diagramType: string): string => {
      ...
      if (type === 'MindMapMax') {
        return `ДЛЯ MERMAID MINDMAP (${diagramType}) - МАКСИМАЛЬНО ДЕТАЛЬНАЯ ИНСТРУКЦИЯ:

КРИТИЧЕСКИ ВАЖНО - ОБЯЗАТЕЛЬНО СЛЕДУЙ ВСЕМ ПРАВИЛАМ:

1. СИНТАКСИС - ОБЯЗАТЕЛЬНО:
   - ВСЕГДА начинай код с ключевого слова "mindmap" на ПЕРВОЙ строке, БЕЗ отступов и пробелов перед ним!
   - НЕПРАВИЛЬНО: "  mindmap" или "\\n  mindmap"
   - ПРАВИЛЬНО: "mindmap" (на первой строке, без пробелов)

2. ОТСТУПЫ - КРИТИЧЕСКИ ВАЖНО:
   - Используй ТОЛЬКО ПРОБЕЛЫ для отступов, НИКОГДА не используй табуляцию (TAB)!
   - Корневой узел: 2 пробела отступа (вторая строка после "mindmap")
   - Каждый следующий уровень: +2 пробела от предыдущего уровня
   - Все узлы одного уровня должны иметь ОДИНАКОВЫЙ отступ
   - ПРАВИЛЬНО: mindmap\\n  root((Корневой узел))\\n    Подтема 1\\n      Деталь 1.1
   - НЕПРАВИЛЬНО: mindmap\\n   root((Корневой узел)) (3 пробела вместо 2)

3. КОРНЕВОЙ УЗЕЛ - ТОЛЬКО ОДИН:
   - В MindMap может быть ТОЛЬКО ОДИН корневой узел!
   - Корневой узел ОБЯЗАТЕЛЬНО на второй строке с отступом 2 пробела
   - Формат: "  root((Название))" или "  Название"
   - Если в описании несколько основных тем, выбери ОДНУ главную как корневой узел, остальные - подтемы первого уровня
   - НЕПРАВИЛЬНО: несколько узлов с отступом 2 пробела (это вызовет ошибку "There can be only one root")

4. ИЕРАРХИЯ:
   - Каждый узел (кроме корневого) ДОЛЖЕН иметь родителя
   - Если узел имеет отступ 2 пробела и это не корневой узел - ОШИБКА!
   - Ошибка "No parent could be found for..." означает, что узел имеет неправильный отступ

5. НАЗВАНИЯ УЗЛОВ:
   - НЕ используй кавычки в названиях: "Название" - НЕПРАВИЛЬНО
   - НЕ используй скобки в названиях (кроме двойных скобок для корневого узла)
   - Можно: буквы, цифры, пробелы, дефисы, подчеркивания
   - Все названия на РУССКОМ языке

6. ПРИМЕРЫ ПРАВИЛЬНОГО КОДА:
...
7. ПРОВЕРКА ПЕРЕД ОТПРАВКОЙ - ОБЯЗАТЕЛЬНО:
   ✅ Код начинается с "mindmap" на первой строке БЕЗ отступов?
   ✅ Только ОДИН корневой узел с отступом 2 пробела?
   ✅ Все остальные узлы имеют отступы кратные 2 (4, 6, 8...)?
   ✅ Каждый узел (кроме корневого) имеет родителя?
   ✅ Нет специальных символов в названиях?
   ✅ Все названия на русском языке?

8. ЧАСТЫЕ ОШИБКИ:
   - "There can be only one root" → несколько узлов с отступом 2 пробела
   - "No parent could be found" → узел имеет неправильный отступ
   - "Syntax error" → неправильный синтаксис, специальные символы, неправильные отступы

ВСЕГДА проверяй код перед отправкой!`;
      }
