...
export default function BpmnViewer({ xml, className = '' }: BpmnViewerProps) {
  ...
  if (error) {
    return (
      <div className={`rounded border border-red-200 bg-red-50 p-4 text-red-700 text-sm ${className}`}>
        Не удалось отобразить диаграмму: {error}. Переключитесь на «Код» для просмотра XML.
      </div>
    );
  }

  return (
    <div className={`bpmn-viewer-in-chat min-h-[360px] rounded border border-gray-200 bg-white overflow-hidden ${className}`}>
