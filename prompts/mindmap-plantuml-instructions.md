...
**КРИТИЧЕСКИ ВАЖНО (ошибка PlantUML error42L):** для левых веток (`-`) нельзя пропускать уровни. Первая левая ветка от корня — всегда ровно **`--`** (два минуса), затем при необходимости `---`, `----` и т.д., каждый раз **не больше чем на один минус** глубже предыдущей левой строки. **Неправильно:** после только правых `+++` сразу писать `----` слева — нужно сначала `--`, потом при необходимости углублять.
