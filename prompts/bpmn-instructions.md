...
- **exclusiveGateway** — ветвление «один из», один входящий поток, несколько исходящих.
- **parallelGateway** — разветвление/слияние параллельных веток.
- **sequenceFlow** — связь между элементами; каждый `sourceRef` и `targetRef` должны совпадать с `id` существующих элементов в том же процессе.
